...
func (r *PlaylistRepository) GetUserPlaylists(ctx context.Context, userId uuid.UUID) ([]domain.Playlsit, error) {
...
`
	...
	if err != nil {
		return nil, fmt.Errorf("Query user playlists: %w", err)
	}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("Rows user playlists: %w", err)
	}
