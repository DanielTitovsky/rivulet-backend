...
func (uh *UsersHttpHandler) RemoveTrackFromFavorite(c *gin.Context) {
	...
	err = uh.userService.RemoveTrackFromFavorite(ctx, *userId, *trackId)
	if err != nil {
		responseHandler.ErrorResponse(err, "Failed to remove track from favorite")
		return
	}

	responseHandler.JSONResponse(app_http_response.Response{
		Status: http.StatusOK,
		Data:   "Track removed from favorite",
	})
}
